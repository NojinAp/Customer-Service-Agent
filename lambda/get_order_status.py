...
def lambda_handler(event, context):
    ...
    for param in parameters:
        ...
        elif param.get("name") == "customer_name":
            customer_name = param.get("value", "").title()
    ...
    if not order_id:
        if not customer_name:
            response_body = {"TEXT": {"body": "order_id or customer name parameter is required"}}
        else:
            sql = f"SELECT * FROM orders WHERE customer_name = '{customer_name}'"
            try:
                results = run_query(sql)
                if not results:
                    response_body = {"TEXT": {"body": f"No order found for customer_name: {customer_name}"}}
                else:
                    response_body = {
                        "TEXT": {
        ...
        try:
            results = run_query(sql)
            if not results:
                response_body = {"TEXT": {"body": f"No order found for order_id: {order_id}"}}
            else:
                order = results[0]
                response_body = {
                    "TEXT": {
